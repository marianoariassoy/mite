...
export default function Layout({ children, params }: { children: React.ReactNode; params: { id: string } }) {
  ...
  return (
    <section id="artist">
      <h1 className="text-3xl lg:text-4xl font-medium underline mb-6">PAULA CASTRO</h1>
      <div className="flex gap-x-4 lg:gap-x-12 gap-y-2 text-sm flex-wrap mb-4">
        {subMenu.map((item) => (
          <Link href={`/artists/${params.id}/${item.url}`} className={`uppercase font-medium hover:underline underline-offset-4 ${section === item.id && "underline"}`} key={item.id} onClick={() => setSection(item.id)}>
      ...
      <div className="fixed left-3 bottom-16 lan z-50">
        <button className="underline-offset-4 hover:underline" onClick={backToTop}>
